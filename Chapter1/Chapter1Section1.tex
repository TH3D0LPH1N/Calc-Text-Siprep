...
\begin{center}  
    \fbox{\fbox{\begin{minipage}{0.96 \textwidth}
        ...
        \begin{center}
            ...
            \\[11pt]
            $\hfill \deriv = \text{``d - y - d - x''} \hfill f'(x) = \text{``f prime of x''} \hfill y' = \text{``y prime''} \hfill$ \\[11pt] 
...
\begin{center}
    \fbox{\fbox{\begin{minipage}{0.96 \textwidth}
        \begin{align*}
            ...
            & \diff \left[cx^n\right] = (cn)x^{n - 1} \\[11pt]
...
\begin{tcolorbox}[example]
    \textbf{Ex 1.1.4: } $y = \sqrt{x^3} + \dfrac{4}{\sqrt{x}} - \sqrt[4]{x^3} + e^4$; find $\deriv$. 
\end{tcolorbox}
\begin{tcolorbox}[solution]
    \textbf{Sol 1.1.4: } \begin{align*}
        ...
        \end{aligned} \\[11pt]
        & \frac{\dd y}{\dd x} \begin{aligned}[t]
...
\hypertarget{Section 1.1} Note in Ex 1.1.4 that $e^4$ is a constant. Therefore, its derivative is $0$. \par
As we have seen, when the variable was in the exponent, we use the Exponential Rules. When the variable was in the base, we used the Power Rule. But what if the variable is in both places, such as $\diff\left[(2x - 1)^{x^2}\right] \forcespace$? It is definitely an exponential problem, but the base is not a constant as the rules above have. The Change of Base Rule allows us to clarify the problem: \begin{align*}
    \diff \left[(2x - 1)^{x^2}\right] = \diff \left[e^{x^2\ln(2x - 1)}\right]
\end{align*} but we will need the \hyperlink{Product Rule}{Product Rule} for this derivative. Therefore, we will save this for later.
...
\begin{tcolorbox}[solution]
    \textbf{Sol 1.1.5: } \begin{align*}
        ...
        & \deriv \begin{aligned}[t]
            & = \frac{\mathrm{d}}{\mathrm{d}x}\left[x^5 - 3x^3 - 4x\right] \\[11pt]
            & = \boxed{5x^4 - 9x^2 - 4} \\
        \end{aligned}
    \end{align*} 
\end{tcolorbox} \vspace{11pt}
...
If you think of the inside function (the $g(x)$) as equaling $u$, we could write the Chain Rule like this: \begin{align*}
    \diff [f(u)] = \dfrac{\dd f}{\dd u} \cdot \dfrac{\dd u}{\dd x}
\end{align*}
...
\fbox{\fbox{\begin{minipage}{0.96 \textwidth}
    \begin{align*}
        & \text{The Power Rule:} && \text{The Exponential Rules:} \\[11pt]
        & \diff \left[u^n\right] = nu^{n - 1} \cdot \dfrac{\dd u}{\dd x} && \diff \left[e^u\right] = e^u \cdot \dfrac{\dd u}{\dd x} \\[11pt]
        & && \diff \left[a^u\right] = \left(a^u \cdot \ln{a}\right) \cdot \dfrac{\dd u}{\dd x} \\
    \end{align*}
    \begin{center}
...
\begin{tcolorbox}[solution]
    \textbf{Sol 1.1.10: } \begin{align*}
        ...
        & \deriv \begin{aligned}[t]
            & = \diff \left[f(u)\right] = f'(u) \cdot \dfrac{\dd u}{\dd x} \\[11pt] 
...
\begin{tcolorbox}[solution]
    \textbf{Sol 1.1.11: }\begin{align*}
        ...
        & \diff \left[f(g(u))\right] \begin{aligned}[t]
            & = f'(g(u)) \cdot g'(u) \cdot \dfrac{\dd u}{\dd x} \\[11pt]
...
\twoquestion{1. $f(x) = x^2 + 3x - 4$}{2. $f(t) = \dfrac{1}{4}\left(t^4 + 8\right)$} \\[11pt]
\twoquestion{3. $y = x^{-\frac{2}{3}}$}{4. $y = 5e^x + 3$} \\[11pt]
\twoquestion{5. $v(r) = \dfrac{4}{3} \pi r^3$}{6. $g(x) = x^2 + \dfrac{1}{x^2}$} \\[11pt]
\twoquestion{7. $y = \dfrac{x^2 + 4x + 3}{\sqrt x}$}{8. $u = \sqrt[3]{t^2} + 2\sqrt{t^3}$} \\[11pt]
\twoquestion{9. $z = \dfrac{A}{y^{10}} + Be^y$}{10. $y = e^{x + 1} + 1$} \\[11pt]

Complete the following. \par

\twoquestion{11. $\diff \left[x^7 - 4\sqrt[8]{x^7} + 7^x - \dfrac{1}{\sqrt[7]{x^4}} + \dfrac{1}{5x}\right]$}{12. $\diff \left[x^6 - 3\sqrt[6]{x^7} + 5^x - \dfrac{1}{\sqrt[3]{x^5}} + \dfrac{1}{8x} \right]$} \\[11pt]
\twoquestion{13. $\diff \left[x^4 - 14\sqrt[7]{x^9} + 8^x - \dfrac{1}{\sqrt[3]{x^7}} + \dfrac{1}{8x}\right]$}{14. $\diff \left[(x - 1)\sqrt{x}\right]$} \\[11pt]
\twoquestion{15. $\dfrac{\mathrm{d}}{\mathrm{d}z} \left[\left(z^2 - 4\right)\sqrt{z^3}\right]$}{16. $\diff \left[\left(x^2 - 4x + 3\right)\sqrt{x^5}\right]$} \\[11pt]
\twoquestion{17. $\tdiff \left[\left(4t^2 + 1\right)\left(3t^3 + 7\right)\right]$}{18. $\diff \left[\left(x^3 + 4x - \pi\right)^{-7}\right]$} \\[11pt]
\twoquestion{19. $\diff \left[\sqrt{3x^2 - 4x + 9}\right]$}{20. $\diff \left[\sqrt[7]{x^3 - 2x}\right]$} \\[11pt]
\twoquestion{21. $\dfrac{\mathrm{d}}{\mathrm{d}y} \left[\dfrac{4y^3 - 2y^2 - 5y}{\sqrt y}\right]$}{22. $\dfrac{\mathrm{d}}{\mathrm{d}v} \left[\dfrac{v^2 - 4v + 7} {2\sqrt v}\right]$} \\[11pt]
\twoquestion{23. $\dfrac{\mathrm{d}}{\mathrm{d}w}\left[\dfrac{7w^2 - 4w + 1}{5w^3}\right]$}{24. $\dfrac{\mathrm{d}}{\mathrm{d}w}\left[\dfrac{5w^2 - 3w - 4}{7w^2}\right]$} \\[11pt]
\twoquestion{25. $f(x) = \sqrt[4]{1 + 2x + x^3}$, find $f'(x)$}{26. $f(x) = \sqrt[5]{\left(\dfrac{1}{x} + 2x + e^x\right)^3}$, find $f'(x)$} \\[11pt]
\twoquestion{27. $f(x) = \left(x^3 + 2x\right)^{37}$, find $f'(x)$}{28. $f(x) = 3x^5 - 5x^3 + 3$, find $f'(x)$} \\[11pt]
\twoquestion{29. $g(2) = 3, \; g'(2) = -4, \; f(x) = e^{g(x)}$, find $f'(2)$}{30. $y = e^{\sqrt x}$, find $\deriv$} \\[11pt]
\twoquestion{31. $f(x) = \sqrt{4 - \dfrac{4}{9}x^2}$, find $f'\left(\sqrt 5\right)$}{32. $f(x) = e^{\sqrt{9 - x^2}}$, find $f'(x)$} \\[11pt]
\twoquestion{33. $v(t) = \sqrt{\left(\dfrac{E(t)}{3} + 3t\right)^{\frac{3}{7}} - 4}$, find $v'(t)$}{34. $v(t) = \sqrt[3]{\left(\dfrac{C(t)}{7} + 4t^2\right)^{\frac{5}{7}} - 1}$, find $v'(t)$} \\[11pt]
