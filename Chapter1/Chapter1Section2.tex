...
\begin{center}
    \fbox{\fbox{\begin{minipage}{0.96\textwidth}
        ...
        \vspace{11pt}
        \begin{align*}
            & \diff[\sin u] = (\cos u) \dfrac{\dd u}{\dd x} && \diff[\csc u] = (-\csc u \cot u) \dfrac{\dd u}{\dd x} \\[11pt] % Line 1
            & \diff[\cos u] = (-\sin u) \dfrac{\dd u}{\dd x} && \diff[\sec u] = (\sec u \tan u) \dfrac{\dd u}{\dd x} \\[11pt] % Line 2
            & \diff[\tan u] = \left(\sec^2 u\right) \dfrac{\dd u}{\dd x} && \diff[\cot u] = \left(-\csc^2 u\right) \dfrac{\dd u}{\dd x} \\[11pt]
        \end{align*}
    \end{minipage}}}
\end{center}
\begin{center}
    \fbox{\fbox{\begin{minipage}{0.96\textwidth}
        ...
        \vspace{11pt}
        \begin{align*}
            & \diff [\ln u] = \left(\dfrac{1}{u}\right) \dfrac{\dd u}{\dd x} && \diff [\log_a u] = \left(\dfrac{1}{u \cdot \ln a}\right) \dfrac{\dd u}{\dd x} \\[11pt]
        \end{align*}
    \end{minipage}}}
\end{center}
...
\begin{center}
    \fbox{\fbox{\begin{minipage}{0.96\textwidth}
        ...
        \vspace{11pt}
        \begin{align*}
            & \diff\left[\sin ^{-1} u\right] = \left(\dfrac{1}{\sqrt{1 - u^2}} \right) \dfrac{\dd u}{\dd x} && \diff\left[\csc^{-1} u\right] = \left(\dfrac{-1}{|u|\sqrt{u^2 - 1}}\right) \dfrac{\dd u}{\dd x} \\[11pt] 
            & \diff\left[\cos ^{-1} u\right] = \left(\dfrac{-1}{\sqrt{1 - u^2}} \right) \dfrac{\dd u}{\dd x} && \diff\left[\sec^{-1} u\right] = \left(\dfrac{1}{|u|\sqrt{u^2 - 1}}\right) \dfrac{\dd u}{\dd x} \\[11pt] 
            & \diff\left[\tan ^{-1} u\right] = \left(\dfrac{1}{u^2+ 1} \right) \dfrac{\dd u}{\dd x} && \diff\left[\cot^{-1} u\right] = \left(\dfrac{-1}{u^2 + 1}\right) \dfrac{\dd u}{\dd x} \\[11pt]
        \end{align*}
    \end{minipage}}}
\end{center}
...
\begin{tcolorbox}[solution]
    \textbf{Sol 1.2.8: } \begin{align*}
        ...
        & \dfrac{\mathrm{d}}{\mathrm{d}x} \left[f^{-1}(x)\right] = \dfrac{1}{f'\left[f^{-1}(x)\right]} = \dfrac{1}{f'\left(g(x)\right)} \\[11pt]
...
\twoquestion{23. $\diff \left[\ln^3 \left(x^2 + 1\right)\right]$}{24. $\diff \left[\ln \left(\sin \left(x^3\right)\right)\right]$} \\[11pt]
...
\twoquestion{35. $y = \sin^{-1} \left(x\sqrt{2}\right),$ find $y'$}{36. $y = \sin^{-1} \left(2x + 1\right),$ find $y'$} \\[11pt]

\textbf{\large{1.2 Free Response Homework Set B}} \par

Find the derivatives of the given functions. Simplify where possible. \par

\twoquestion{1. $y = \cos^{-1} \left(e^{3z}\right)$}{2. $y = \tan^{-1} \left(\sqrt{x^2 - 1}\right)$} \\[11pt]
\twoquestion{3. $y = \sec^{-1} (4x) + \csc^{-1} (4x)$}{4. $f(x) = \ln \left(\tan^{-1} (5x)\right)$} \\[11pt]
\twoquestion{5. $g(w) = \sin^{-1} (5w) + \cos^{-1} (5w)$}{6. $f(t) = \sec^{-1} \left(\sqrt{9 + t^2}\right)$} \\[11pt]

Complete the following. \par

\twoquestion{7. $\dfrac{\dd }{\dd \theta} \left[e^{\csc (\theta)} + \ln \left(\cot \left(\theta^2\right)\right) - \sec (\theta) \right]$}{8. $\diff \left[\ln \left(\sec^3 \left(x^3 + 5\ln x + 7\right)\right)\right]$} \\[11pt]
\twoquestion{9. $\diff \left[\ln \left(\tan^3 \left(x^2 + 5e^x + 7\right)\right)\right]$}{10. $\diff \left[\dfrac{\cos \left(\ln \left(5x^2\right)\right)}{\sin \left(\ln \left(5x^2\right)\right)} \right]$} \\[11pt]
\twoquestion{11. $\diff \left[\ln \left(\sqrt{x^2 + 4x -5}\right)\right]$}{12. $\tdiff \left[\sin^{5} \left(\ln \left(7t + 3\right)\right)\right]$} \\[11pt]
\twoquestion{13. $\diff \left[\csc \left(\ln \left(7x^2 + x\right)\right)\right]$}{14. $\diff \left[\ln \left(\sqrt{e^{4t^2 + 6}}\right)\right]$} \\[11pt]
\twoquestion{15. $\diff \left[\sqrt{9x - 27x^2 + \dfrac{5}{x^3}}\right]$}{16. $\diff \left[\sec (5x) + \cot \left(e^x\right) - 10\ln x\right]$} \\[11pt]
\twoquestion{17. $z = \ln \left(\cos (t)\right) + \sec \left(e^t\right) + 7\pi^2$, find $\dfrac{\dd z}{\dd t}$}{18. $z = \ln \left(\tan (t)\right) + \sin \left(e^t\right) + 7\pi^2$, find $\dfrac{\dd z}{\dd t}$} \\[11pt]
\twoquestion{19. $z = \ln \left(\cot (\theta)\right) + \sec \left(\ln \theta\right) + 7\pi^2$, find $\dfrac{\dd z}{\dd \theta}$}{20. $z = \ln \left(\cos (\theta)\right) + \sin \left(\ln \theta\right) + 7\pi^2$, find $\dfrac{\dd z}{\dd \theta}$} \\[11pt]
...
\begin{questions}
    \question If $y = \sin^{-1} \left(e^{3\theta}\right)$, then $\dfrac{\dd y}{\dd \theta} = $ \\
