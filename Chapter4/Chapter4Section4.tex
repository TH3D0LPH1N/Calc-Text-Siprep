...
\onequestion{2. The sign pattern for the derivative of $H(x)$ is given.} \begin{center}
    \begin{tikzpicture}[baseline=(current bounding box.center)]
        ...
        \node at (6,0.5) {$0$};
        \node at (7,0.5) {$+$};
        \node at (-0.5, 0.5) {$\dfrac{d^2H}{dx^2}$};
    \end{tikzpicture}
\end{center}
...
\onequestion{13. $y = 2x - x^{\frac{2}{3}}$} \\[11pt]
...
\begin{questions}
    ...
    \question If $\diff [f(x)] = g(x)$ and $\diff [g(x)] = f(3x)$, then $\dfrac{d^2}{dx^2} \left[f\left(x^2\right)\right]$ is \\
