...
As we saw with the Riemann sums, in $\int f(x) \,\mathrm{d}x$, $f(x_i) \cdot \,\mathrm{d}x$ is the area of a rectangle (height times base). The $\int \forcespace$ is the sum of the areas. The main concept in these ``accumulation of rates'' problem is that since a definite integral is a sum of values, the integral of a rate of change over some time $t$ equals the total change over that time $t$. \par
...
\begin{tcolorbox}[example]
    ...
    \begin{enumerate}[label=\hspace{11pt}(\alph*), align=left, leftmargin=*, labelsep=0.25em]
        ...
        \item Let $H(t) = \int_8^t E(x) - L(x) \,\mathrm{d}x$ for $8 \leq t \leq 24$. The value of $H(16)$ to the nearest whole number is $5023$. Find the value of $H'(16)$ and explain the meaning of $H(16)$ and $H'(16)$ in the context of the amusement park. \\
...
\hypertarget{key phrases and units}{}
\begin{center}
    \fbox{\fbox{\begin{minipage}{0.96\textwidth}
        ...
        \end{center}
        \begin{alignat*}{2}
            & \text{Total change:} &\qquad& \int_a^b R(t) \,\mathrm{d}t \text{ or } \int_a^t (\text{incoming rate} - \text{outgoing rate}) \,\mathrm{d}x \\[11pt]
            & \text{Total rate of change:} &\qquad& \text{incoming rate} - \text{outgoing rate} \\[11pt]
            & \text{Total amount:} &\qquad& \text{initial value} + \int_a^t (\text{incoming rate} - \text{outgoing rate}) \,\mathrm{d}x \\[11pt]
            & \text{Instantaneous rate of change:} &\qquad& \dfrac{\dd x}{\dd t} \text{ or } R(t) \\[11pt]
            & \text{Average rate of change:} &\qquad& \dfrac{f(b) - f(a)}{b - a} \\[11pt]
            & \text{Average value of $f(x)$:} &\qquad& \dfrac{1}{b - a}\int_a^b f(x) \,\mathrm{d}x \\[11pt]
            & \text{Amount increase/decrease:} &\qquad& \text{rate of change positive/negative} \\[11pt]
            & \text{Rate of change inc/dec:} &\qquad& \tdiff\text{[rate of change] positive/negative} \\
        \end{alignat*}
    \end{minipage}}}
\end{center}
...
One technique that is super useful in accumulation of rates problem is unit analysis. By analyzing the units of the problem, we are able to interpret what the question is asking, and provide an appropriate answer. For instance, if $R(t)$ is measured in miles per hour, \begin{align*}
    \int_a^b R(t) \,\mathrm{d}t = \int_a^b \dfrac{\text{miles}}{\text{hour}}(\text{hours}) = \text{miles}
\end{align*}
...
\hypertarget{tips for explaining answers}{}
\begin{center}
    \fbox{\fbox{\begin{minipage}{0.96\textwidth}
    \vspace{11pt}
    \begin{center}
...
\begin{tcolorbox}[solution]
    ...
    a) Since $E(t)$ is a rate in people per hour, the number of people who have entered the park will be an integral from $t = 8$ to $t = 16$. Therefore, \begin{align*}
        \text{Total entered} &= \int_8^{16} E(t) \,\mathrm{d}t \\[11pt]
        & = 6126.105 \\[11pt]
        & \approx \boxed{6126 \text{ people}}
    \end{align*}
    b) Since there are different entry fees for different times of day, we need to determine how many people paid each fee. So, we will have to do $\int_8^{16} E(t) \,\mathrm{d}t$ and $\int_{16}^{24} E(t) \,\mathrm{d}t$. Conveniently, we know that $\int_8^{16} E(t) \,\mathrm{d}t = 6126$ people from the previous problem, so we can just find $\int_{16}^{24} E(t) \,\mathrm{d}t$. \begin{align*}
        \int_{16}^{24} E(t) \,\mathrm{d}t \approx 1808 \text{ people}
    \end{align*}
    ...
    Note how each function increases and then decreases. Since $H(t) = \int_8^t E(x) - L(x) \,\mathrm{d}x$, we can use the Fundamental Theorem of Calculus to determine the derivative. \begin{align*}
        H'(t) &= \tdiff\int_8^t E(x) - L(x) \,\mathrm{d}x \\[11pt]
        & = E(t) - L(t) \\[11pt]
        & = E(16) - L(16) \\[11pt]
        & = 14
    \end{align*}
...
\begin{tcolorbox}[example]
    ...
    \begin{enumerate}[label=\hspace{11pt}(\alph*), align=left, leftmargin=*, labelsep=0.25em]
        \item $\int_0^{12} R(t) \,\mathrm{d}t = 1440$ cats. Using the correct units, explain the meaning of this result in context of the problem. \\
        \item Assume $A'(10.3) = -28.008$. Using the correct units, explain the meaning of $A'(10.3)$ in context of the problem. \\
        \item $C(t) = 131 + \int_0^{12} R(t) - A(t) \,\mathrm{d}t = 71$. Using the correct units, explain the meaning of this result in context of the problem. \\
        \item Using the correct units, explain the meaning of $\dfrac{1}{t}\int_0^t R(x) \,\mathrm{d}x$ in the context of the problem. \\
...
\begin{tcolorbox}[solution]
    \textbf{Sol 3.4.2: } \par
    a) $R(t)$ is the rate at which healthy cats per month are received at PHS. Therefore, $\int_0^{12} R(t) \,\mathrm{d}t \forcespace$ would be the total number of cats received over this time period. So, \textbf{1440 healthy cats were received at PHS between $t = 0$ and $t = 12$ (or, during the 12 months of 2019).} \par
    \vspace{11pt}
    ...
    \vspace{11pt}
    c) $C(t) = 131 + \int_0^t R(x) - A(x) \,\mathrm{d}x$ represents the total number of cats and kittens sheltered at PHS at any time $t$. So, we can say that \textbf{the number of cats and kittens sheltered at PHS at the end of the 12 months of 2019 is 71 cats and kittens.} \par
    \vspace{11pt}
    d) $\dfrac{1}{t}\int_0^t R(x) \,\mathrm{d}x$ is the average number of cats and kittens being received at any time $t$. Therefore, \textbf{$\dfrac{1}{t}\int_0^t R(x) \,\mathrm{d}x \forcespace$ represents the average number of cats and kittens, in cats and kittens per month, that have been received at PHS during the interval $[0, t]$ of 2019.} \par
    \vspace{11pt}
...
\begin{tcolorbox}[solution]
    ...
    a) To find the total amount removed over the time interval, we integrate the rate $R(t)$ over $0 \leq t \leq 6$. \begin{align*}
        \text{Amount removed} &= \int_0^6 \left(2 + 5\sin \left(\dfrac{4\pi}{25}t\right) \right) \,\mathrm{d}t \\[11pt]
        & = \int_0^6 R(t) \,\mathrm{d}t \\[11pt]
    ...
    b) Let $P(t)$ represent the total amount of oil in the reaction vessel at time $t$. Initially, $P(0) = 2500$ gallons. The net change of oil is production minus removal, which is $S(t) - R(t)$. Since the variable $t$ is the upper boundary, we need to use a dummy variable $x$ in the integrand. This gives us the expression \begin{align*}
        \boxed{P(t) = 2500 + \int_0^t (S(x) - R(x)) \,\mathrm{d}x}.
    \end{align*}
...
\end{align*}
\begin{enumerate}[label=\hspace{11pt}(\alph*), align=left, leftmargin=*, labelsep=0.25em]
    \item Using the correct units, explain the meaning of $\int_0^{50} (B(t) - D(t)) \,\mathrm{d}t$. 
    \item Using the correct units, explain the meaning of $\dfrac{1}{25} \int_0^{25} B(t) \,\mathrm{d}t$. 
...
\begin{enumerate}[label=\hspace{11pt}(\alph*), align=left, leftmargin=*, labelsep=0.25em]
    \item $\int_0^{2.5} R(t) \,\mathrm{d}t = 0.029$. Using the correct units, explain the meaning of this result. 
    ...
    \item Using the correct units, explain the meaning of $\dfrac{2}{5}\int_0^{2.5} R(t) \,\mathrm{d}t$. 
...
\begin{questions}
    ...
    \question For $t \geq 0$ hours, $H$ is a differentiable function of $t$ that gives the temperature, in degrees Celsius, at an Arctic weather station. Which of the following is the best interpretation of $\int_0^t H(x) \,\mathrm{d}x$. \\
    ...
    \begin{oneparchoices}
        \choice $\int_0^3 R(t) \,\mathrm{d}t$
        \choice $\dfrac{1}{3}\int_0^3 R(t) \,\mathrm{d}t$
        \choice $\int_0^3 R'(t) \,\mathrm{d}t$
        \choice $R(3) - R(0)$
    ...
    \question The rate at which ice is melting in a pond is given by $\dfrac{\dd V}{\dd t} = \sqrt{1 + 2^t}$, where $V$ is the volume of the ice in cubic feet and $t$ is the time in minutes. The amount of ice which has melted in the first five minutes is \\
