...
\begin{questions}
    ...
    %Question 3
    \question Based on the information below, find $\int_1^{-2} (3g(x) + 2f(x)) \, dx$ \begin{align*}
    ...
    %Question 4
    \question For $t \geq 0$ hours, $H$ is a differentiable function of $t$ that gives the rate of change in temperature, in degrees Celsius per hour, at an Arctic weather station. In what units would $\dfrac{1}{t}\int_0^t H'(x) \, dx$ be measured? \\
    ...
    %Question 5
    \question $\int_0^1 x^2\tan \left(x^3\right) \, dx = $ \\
    ...
    \begin{oneparchoices}
        \choice $\int_0^t S(x) \, dx$
        \choice $\dfrac{1}{t}\int_0^t S(x) \, dx$
        \choice $S(t)$ \\[11pt]
        \makebox[0.25\textwidth] \choice $S'(t)$
        \makebox[0.25\textwidth] \choice $\dfrac{\displaystyle\int_0^t S(x) \, dx - 50}{t - 0}$
    \end{oneparchoices} \par \horizontalline
    ...
    \end{center}
    \question The approximate value of $\int_0^10 f(x) \, dx$, using 5 right-hand rectangles with equal width, is \\
...
\begin{questions}
    ...
    \begin{parts}
        \part Show the $u$-sub setup for $\int_0^{\sqrt{10}} g(x) \, dx$ and solve it by calculator.
    ...
    \begin{parts}
       \part Using a right-hand Riemann approximation, determine the approximate number of sacks Bryant Young had during the four years. Round to the nearest whole number. 
        ...
        \begin{array}{|c|c|c|c|c|c|}
            \hline
            g \text{ (games completed)} & 28 & 44 & 56 & 68 & 84 \\ \hline
            B(g) \text{ (sacks per game)} & 0.5 & 0.72 & 0.33 & 0.79 & 0.69 \\
            \hline
        \end{array}
        \end{align*}

        \part The table above shows Nick Bosa's sack rates over his first four seasons. Use a right-hand Riemann approximation to approximate $\dfrac{1}{50}\int_0^{50} B(g) \, dg \forcespace$. Using the correct units, explain the result in the context of the problem.
