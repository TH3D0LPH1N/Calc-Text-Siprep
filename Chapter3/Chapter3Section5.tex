...
\begin{tcolorbox}[example]
    \textbf{Ex 3.5.1: } Use a left hand Riemann sum with four equal sub-intervals to approximate $\int_1^5 f(x) \,\mathrm{d}x$ given the table of values below. \begin{align*}
...
\begin{tcolorbox}[solution]
    ...
    So, to find the areas of the rectangle, we multiply the width of the rectangle by the height. The width of the rectangle is the difference between adjacent $x$-values. Because we are asked to find a left hand Riemann sum, we are gonna start with the leftmost height. Therefore, we have \begin{align*}
        \int_1^5 x^2 \,\mathrm{d}x \approx (2 - 1) \cdot 1 + (3 - 2) \cdot 4 + (4 - 3) \cdot 9 + (5 - 4) \cdot 16 = \boxed{30}
    \end{align*}
...
\begin{tcolorbox}[example]
    \textbf{Ex 3.5.2: } Use a right hand Riemann sum with the sub-intervals defined below to approximate $\int_2^{14} f(x) \,\mathrm{d}x$ given the table of values below. \begin{align*}
        \arraycolsep=22pt\def\arraystretch{1.5}
...
\begin{tcolorbox}[solution]
    ...
    Now, as this time we are asked to find a right hand Riemann sum, we start from the right. So, we have \begin{align*}
        \int_2^{14} f(x) \,\mathrm{d}x \approx (14 - 10) \cdot 12 + (10 - 5) \cdot 7 + (5 - 2) \cdot 10 = \boxed{113}
    \end{align*}
\end{tcolorbox} 
...
\begin{tcolorbox}[example]
    \textbf{Ex 3.5.3: } The rate of consumption, in gallons per minute, recorded during an airplane flight is given by a twice differentiable and strictly increasing function $R(t)$. A table of selected values for $R(t)$ for the time interval $0 \leq t \leq 90$ is shown below. \begin{align*}
        ...
        \end{array}
    \end{align*}
    Use the Riemann sum with midpoint rectangles and the sub-intervals given by the table to approximate the value of $\int_0^{120} R(t) \,\mathrm{d}t \forcespace$.
\end{tcolorbox}
\begin{tcolorbox}[solution]
...
\begin{tcolorbox}[example]
    \textbf{Ex 3.5.4: } Use the midpoint rule and the given data to approximate the value of $\int_0^{2.6} f(x) \,\mathrm{d}x$. \begin{align*}
...
\begin{tcolorbox}[solution]
    \textbf{Sol 3.5.4: } For this example, we simply follow the steps in \textbf{Ex 3.5.3}. This gives us \begin{align*}
        \int_0^{2.6} f(x) \,\mathrm{d}x \approx (0.8 - 0) \cdot 2.3 + (1.6 - 0.8) \cdot 4.3 + (2.6 - 1.6) \cdot 5.9 = \boxed{11.18}
    \end{align*}
\end{tcolorbox}
...
\begin{tcolorbox}[example]
    \textbf{Ex 3.5.5: } Using the table of data from \textbf{Ex 3.5.4} approximate $\int_0^{2.6} f(x) \,\mathrm{d}x$ with 6 trapezoids.
\end{tcolorbox}
\begin{tcolorbox}[solution]
    ...
    to calculate this Riemann sum: \begin{align*}
        \int_0^{2.6} f(x) \,\mathrm{d}x & \approx \dfrac{3.5 + 2.3}{2} \cdot (0.4 - 0) + \dfrac{2.3 + 3.2}{2} \cdot (0.8 - 0.4) + \dfrac{3.2 + 4.3}{2} \cdot (1.2 - 0.8) \\[5.5pt]
...
Now, let's start writing out the Riemann sum in its complete form. We do the method as we described in $\textbf{Ex 3.5.5}$, taking adjacent $x-values$ as the heights and adjacent pairs of $f(x)$-values as the bases. Therefore, we get{\small{\begin{align*}
    \int_a^b f(x) \,\mathrm{d}x \approx \dfrac{f(x_0) + f(x_1)}{2}\left(\dfrac{b - a}{n}\right) + \dfrac{f(x_1) + f(x_2)}{2}\left(\dfrac{b - a}{n}\right) + \cdots + \dfrac{f(x_{n - 1}) + f(x_n)}{2}\left(\dfrac{b - a}{n}\right).
\end{align*}}}
Factoring out $\dfrac{b - a}{n}$ and simplifying the fraction gives us\begin{align*}
    \int_a^b f(x) \,\mathrm{d}x \approx \left(\dfrac{b - a}{n}\right)\left(\dfrac{f(x_0) + f(x_1) + f(x_1) + f(x_2) + \cdots f(x_{n - 1}) + f(x_n)}{2}\right).
\end{align*}
We can simplify this even further to \begin{align*}
    \int_a^b f(x) \,\mathrm{d}x \approx \left(\dfrac{b - a}{2n}\right)\left(f(x_0) + 2f(x_1) + 2(fx_2) + \cdots + 2f(x_{n - 1}) + f(x_n)\right).
\end{align*}
...
\begin{tcolorbox}[solution]
    \textbf{Sol 3.5.6: } Note that this question is asking for the \textit{average value} of the function. Therefore, we will have to use our average value formula. \begin{align*} 
        f_{avg} &= \dfrac{1}{b - a}\int_a^b f(x) \,\mathrm{d}x \\[11pt]
        & = \dfrac{1}{70 - 10}\int_{10}^{70} f(x) \,\mathrm{d}x \\[11pt]
...
\begin{enumerate}[label=\hspace{11pt}(\alph*), align=left, leftmargin=*, labelsep=0.25em]
    \item Find an approximation for $\int_0^{60} v(t) \,\mathrm{d}t$ using midpoint rectangles. 
    \item Find an approximation for $\int_0^{30} v(t) \,\mathrm{d}t$ using trapezoids. 
    \item Find an approximation for $\int_{30}^{60} v(t) \,\mathrm{d}t$ using left-hand rectangles.
    \item Find an approximation for $\int_{0}^{40} v(t) \,\mathrm{d}t$ using right-hand rectangles.
\end{enumerate} \vspace{11pt}
...
Use this information to answer each of the questions below. Make sure you express your answer in correct units.
\begin{enumerate}[label=\hspace{11pt}(\alph*), align=left, leftmargin=*, labelsep=0.25em]
    \item Find an approximation for $\int_0^{48} V(t) \,\mathrm{d}t$ using midpoint rectangles. 
    \item Find an approximation for $\int_0^{16} V(t) \,\mathrm{d}t$ using right-hand rectangles. 
\end{enumerate} \vspace{11pt}
...
Use this information to answer each of the questions below. Make sure you express your answer in correct units.
\begin{enumerate}[label=\hspace{11pt}(\alph*), align=left, leftmargin=*, labelsep=0.25em]
    \item Find an approximation for $\int_0^{360} v(t) \,\mathrm{d}t$ using left-hand rectangles. 
    \item Find an approximation for $\int_0^{220} v(t) \,\mathrm{d}t$ using trapezoids. 
\end{enumerate} \vspace{11pt}
...
Use this information to answer each of the questions below. Make sure you express your answer in correct units.
\begin{enumerate}[label=\hspace{11pt}(\alph*), align=left, leftmargin=*, labelsep=0.25em]
    \item Find an approximation for $\int_0^{24} v(t) \,\mathrm{d}t$ using midpoint rectangles. 
    \item Find an approximation for $\int_0^{16} v(t) \,\mathrm{d}t$ using trapezoids. 
\end{enumerate} \vspace{11pt}
...
Use this information to answer each of the questions below. Make sure you express your answer in correct units.
\begin{enumerate}[label=\hspace{11pt}(\alph*), align=left, leftmargin=*, labelsep=0.25em]
    \item Find an approximation for $\int_0^{20} V(t) \,\mathrm{d}t$ using trapezoids. 
    \item Find an approximation for $\int_0^{20} V(t) \,\mathrm{d}t$ using left-hand rectangles. 
\end{enumerate} \vspace{11pt}
...
\onequestion{9. Use (a) the trapezoidal rule and (b) the midpoint rule to approximate $\int_0^2 \sqrt[4]{1 + x^2} \,\mathrm{d}x$ with the specified value of $n = 8$ (8 intervals).} \\[11pt]
\onequestion{10. Use (a) the trapezoidal rule and (b) the midpoint rule to approximate $\int_1^2 \dfrac{\ln x}{1 + x} \,\mathrm{d}x$ with the specified value of $n = 10$ (10 intervals).} \\[11pt]
...
\begin{questions}
    ...
    \begin{oneparchoices}
        \choice $\int_1^3 f(x) \,\mathrm{d}x$ \\[11pt]
        \makebox[0.035\textwidth] \choice Left-hand Riemann approximation of $\int_1^3 f(x) \,\mathrm{d}x$. \\[11pt]
        \makebox[0.035\textwidth] \choice Right-hand Riemann approximation of $\int_1^3 f(x) \,\mathrm{d}x$. \\[11pt]
        \makebox[0.035\textwidth] \choice Midpoint Riemann approximation of $\int_1^3 f(x) \,\mathrm{d}x$. \\[11pt]
        \makebox[0.035\textwidth] \choice Trapezoidal approximation of $\int_1^3 f(x) \,\mathrm{d}x$.
    ...
    \question The function $f$, with some values represented in the table below, is continuous and differentiable on the closed interval $[3, \, 12]$. \begin{align*}
        ...
        \end{array}
    \end{align*}
    What is the right Riemann approximation of $\int_3^{12} f(x) \,\mathrm{d}x$? \\
    ...
    \end{center}
    The approximate value of $\int_0^8 f(x) \,\mathrm{d}x$, using eight right-hand rectangles with equal widths is \\
    ...
    \begin{center}
        \includegraphics[width = 0.55\textwidth]{\graphicsdir Chapter 3 Graphics/3.5-Graphic14.png}
    \end{center}
    The approximate value of $\int_0^8 f(x) \,\mathrm{d}x$, using eight left-hand rectangles with equal widths, is \\
    ...
    \begin{center}
        \includegraphics[width = 0.55\textwidth]{\graphicsdir Chapter 3 Graphics/3.5-Graphic14.png}
    \end{center}
    The approximate value of $\int_0^8 f(x) \,\mathrm{d}x$, using eight trapezoids with equal widths, is \\
    ...
    \begin{center}
        \includegraphics[width = 0.55\textwidth]{\graphicsdir Chapter 3 Graphics/3.5-Graphic14.png}
    \end{center}
    The approximate value of $\int_0^8 f(x) \,\mathrm{d}x$, using four midpoint rectangles with equal widths, is \\
    ...
    \question Let $f$ be a differentiable function on the closed interval $[2, \, 14]$ and which has values shown on the table below. \begin{align*}
        ...
        \end{array}
    \end{align*}
    Using the sub-intervals defined by the table, estimate $\int_2^{14} f(x) \,\mathrm{d}x$ using a right-hand Riemann approximation. \\
    ...
    \question The following table lists the known values of a function $f(x)$. \begin{align*}
        ...
        \end{array}
    \end{align*}
    Use the trapezoidal rule to approximate $\int_1^5 f(x) \,\mathrm{d}x$. \\
