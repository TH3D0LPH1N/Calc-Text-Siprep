...
\begin{tcolorbox}[example]
    \textbf{Ex 3.2.1: } Evaluate $\int_0^2 t^2\sqrt{t^3 + 1} \,\mathrm{d}x$.
\end{tcolorbox}
\begin{tcolorbox}[solution]
    \textbf{Sol 3.2.1: } Now, this problem may look just like a regular $u$-substitution problem that we did in the previous chapter. However, when we switch our integration variable to $u$, we also need to make sure to switch our definite integration boundaries to match. Let's see what that means in the solution below. \begin{align*}
        \int_0^2 t^2\sqrt{t^3 + 1} \,\mathrm{d}t &= \dfrac{1}{3}\int_0^2 3t^2\sqrt{t^3 + 1} \,\mathrm{d}t \\[5.5pt]
        \curvedarrow u = t^3 + 1 &\bargap \curvedarrow \dd u = 3t^2 \,\mathrm{d}t
    \end{align*}
    ...
    Now, we can continue integrating! (Note the boundary change in red.) \begin{align*}
        \dfrac{1}{3}\int_{\textcolor{red}{0}}^{\textcolor{red}{2}} 3t^2\sqrt{t^3 + 1} \,\mathrm{d}t &= \dfrac{1}{3}\int_{\textcolor{red}{1}}^{\textcolor{red}{9}} \sqrt{u} \,\mathrm{d}u \\[5.5pt]
...
\begin{tcolorbox}[example]
    \textbf{Ex 3.2.2: } Evaluate $\int_0^{\sqrt{\pi}} x\cos \left(x^2\right) \,\mathrm{d}x$.
\end{tcolorbox}
\begin{tcolorbox}[solution]
    \textbf{Sol 3.2.2: } \begin{align*}
        \curvedarrow u = x^2 &\bargap \curvedarrow \dd u = 2x \,\mathrm{d}x \\[5.5pt]
        u(0) = (0)^2 = 0 &\bargap u\left(\sqrt{\pi}\right) = \left(\sqrt{\pi}\right)^2 = \pi \\[5.5pt]
        & = \dfrac{1}{2}\int_0^{\pi} \cos (u) \,\mathrm{d}u \\[5.5pt]
...
\begin{tcolorbox}[example]
    \textbf{Ex 3.2.3: } Evaluate $\int_1^2 \dfrac{e^{\frac{1}{x}}}{x^2} \,\mathrm{d}x$.
\end{tcolorbox}
\begin{tcolorbox}[solution]
    \textbf{Sol 3.2.3: } \begin{align*}
        \curvedarrow u = \dfrac{1}{x} &\bargap \curvedarrow \dd u = -\dfrac{1}{x^2} \\[5.5pt]
        u(1) = \dfrac{1}{(1)} = 1 &\bargap u(2) = \dfrac{1}{(2)} = \dfrac{1}{2} \\[5.5pt]
        \int_1^2 \dfrac{e^{\frac{1}{x}}}{x^2} \,\mathrm{d}x & = -\int_1^{\frac{1}{2}} e^u \,\mathrm{d}u \\[5.5pt]
...
\begin{tcolorbox}[example]
    \textbf{Ex 3.2.4: } Evaluate $\int_1^{\sqrt{13}} \dfrac{x}{x^2 + 3} \,\mathrm{d}x$.
\end{tcolorbox}
\begin{tcolorbox}[solution]
    \textbf{Sol 3.2.4: } \begin{align*}
        \curvedarrow u = x^2 + 3 &\bargap \curvedarrow \dd u = 2x \,\mathrm{d}x \\[5.5pt]
        u(1) = (1)^2 + 3 = 4 &\bargap u\left(\sqrt{13}\right)^2 + 3 = 16 \\[5.5pt]
        \int_1^{\sqrt{13}} \dfrac{x}{x^2 + 3} \,\mathrm{d}x &= \dfrac{1}{2}\int_1^{\sqrt{13}} \dfrac{2x}{x^2 + 3} \,\mathrm{d}x \\[5.5pt]
        & = \dfrac{1}{2}\int_4^{16} \dfrac{1}{u} \,\mathrm{d}u \\[5.5pt]
...
\begin{center}
    \fbox{\fbox{\begin{minipage}{0.96\textwidth}
        ...
        The average value of a function $f$ on a closed interval $[a, \, b]$ is defined by \begin{align*}
            f_{avg} = \dfrac{1}{b - a}\int_a^b f(x) \,\mathrm{d}x \\
        \end{align*}
    \end{minipage}}}
\end{center}
...
If we look at this formula in the context of the Fundamental Theorem of Calculus, it will start to make a little more sense. \begin{align*}
    \int_a^b f(x) \,\mathrm{d}x &= F(b) - F(a) \\[5.5pt]
    \dfrac{1}{b - a}\int_a^b f(x) \,\mathrm{d}x &= \dfrac{1}{b - a}(F(b) - F(a)) \\[5.5pt]
    & = \dfrac{F(b) - F(a)}{b - a}
\end{align*}
...
\begin{tcolorbox}[solution]
    ...
    Then, we can substitute in the function and the interval. \begin{align*}
        f_{avg} = \dfrac{1}{5 - 0}\int_0^5 \left(x^2 + 1\right) \,\mathrm{d}x
    \end{align*}
...
\twoquestion{1. $\int_0^1 x^2\left(1 + 2x^3\right)^5 \,\mathrm{d}x$}{2. $\int_0^1 x^3\left(x^4 + 5\right)^3 \,\mathrm{d}x$} \\[11pt]
\twoquestion{3. $\int_{-1}^1 x\sqrt{4 - x^2} \,\mathrm{d}x$}{4. $\int_1^2 \dfrac{x^2}{\sqrt[3]{9 - x^3}} \,\mathrm{d}x$} \\[11pt]
\twoquestion{5. $\int_0^3 \dfrac{10t + 15}{\sqrt[4]{t^2 + 3t + 1}} \,\mathrm{d}t$}{6. $\int_1^2 \dfrac{x + 1}{\sqrt{x^2 + 2x + 4}} \,\mathrm{d}x$} \\[11pt]
\twoquestion{7. $\int_1^3 \dfrac{5t}{t^2 + 1} \,\mathrm{d}t$}{8. $\int_{-1}^2 \dfrac{1}{2x + 5} \,\mathrm{d}x$} \\[11pt]
\twoquestion{9. $\int_{\sqrt{3}}^2 ye^{y^2 - 3} \,\mathrm{d}y$}{10. $\int_0^1 \dfrac{v^2}{8 - v^3} \,\mathrm{d}v$} \\[11pt]
\twoquestion{11. $\int_3^{e^2 + 2} \dfrac{1}{x - 2} \,\mathrm{d}x$}{12. $\int_0^{\frac{\pi}{3}} \dfrac{\sin (\theta)}{\cos^2 (\theta)} d\theta$} \\[11pt]
\twoquestion{13. $\int_e^{e^4} \dfrac{1}{x\sqrt{\ln x}} \,\mathrm{d}x$}{14. $\int_0^{\pi} \sec^2 \left(\dfrac{t}{4}\right) \,\mathrm{d}t$} \\[11pt]
\twoquestion{15. $\int_0^{\pi} \dfrac{\sin (x)}{2 - \cos (x)} \,\mathrm{d}x$}{16. $\int_2^4 \dfrac{1}{x\ln x} \,\mathrm{d}x$} \\[11pt]
\twoquestion{17. $\int_0^{\ln 2} \dfrac{e^x}{1 + e^{2x}} \,\mathrm{d}x$}{18. $\int_{\frac{\pi}{6}}^{\frac{\pi}{2}} \cos^5 (x)\sin (x) \,\mathrm{d}x$} \\[11pt]
\twoquestion{19. $\int_0^{\frac{\pi}{8}} \sec^2 (2x) \,\mathrm{d}x$}{20. $\int_{e^{\frac{\pi}{4}}}^{e^{\frac{\pi}{2}}} \dfrac{\csc^2 (\ln y)}{y} \,\mathrm{d}y$} \\[11pt]
\twoquestion{21. $\int_0^{\pi} \dfrac{\cos (x)}{2 + \sin (x)} \,\mathrm{d}x$}{22. $\int_0^{\pi} \dfrac{\sin (y)}{2 + \cos (y)} \,\mathrm{d}y$} \\[11pt]
\twoquestion{23. $\int_0^{\sqrt{\frac{\pi}{4}}} m\sec \left(m^2\right)\tan \left(m^2\right) \,\mathrm{d}m$}{24. $\int_0^{\frac{\pi}{4}} \sec^2 (x)\tan^3 (x) \,\mathrm{d}x$} \\[11pt]
\twoquestion{25. $\int_{\frac{\pi}{2}}^{\pi} \cos^9 (x)\sin (x) \,\mathrm{d}x$}{26. $\int_0^{\pi} \cos^6 \left(\dfrac{x}{2}\right)\sin \left(\dfrac{x}{2}\right) \,\mathrm{d}x$} \\[11pt]
\twoquestion{27. $\int_{\pi}^{2\pi} \cos \left(\dfrac{1}{2}\theta\right) \, d\theta$}{28. $\int_2^{e^3 + 1} \dfrac{(\ln (x - 1))^4}{x - 1} \,\mathrm{d}x$} \\[11pt]
\twoquestion{29. $\int_0^{e^2 - 1} \dfrac{1}{x + 1} \,\mathrm{d}x$}{30. $\int_5^{e^3 + 4} \dfrac{1}{x - 4} \,\mathrm{d}x$} \\[11pt]
...
\begin{questions}
    \question $\int_1^4 \dfrac{1}{\left(1 + \sqrt{x}\right)^2\sqrt{x}} \,\mathrm{d}x$ \\
    ...
    \question If $\int_1^4 h(x) \,\mathrm{d}x = 6$, then $\int_1^4 h(5 - x) \,\mathrm{d}x = $ \\
    ...
    \question $\int_e^{e^2} \dfrac{1}{x \ln x} \,\mathrm{d}x = $ \\
    ...
    \begin{oneparchoices}
        ...
        \makebox[0.2\textwidth] \choice \begin{tikzpicture}
            \begin{axis}[
                axis lines = middle,
                xlabel = $x$,
                ylabel = $y$,
            ...
            \end{axis}
        \end{tikzpicture}
        \makebox[0.15\textwidth] \choice \begin{tikzpicture}
            \begin{axis}[
                axis lines = middle,
                xlabel = $x$,
                ylabel = $y$,
