...
In \textbf{Ex 3.2.2}, we found that $\int_0^{\sqrt{\pi}} x\cos \left(x^2\right) \,\mathrm{d}x = 0 \forcespace$. But, there's clearly area under the curve, so how can the integral equal both the area and 0? Well, as it turns out, because the integral was created from rectangles with width $\dd x$ and height $f(x)$, a negative $f(x)$ will result in a rectangle with ``negative area.'' Take a look at the following graph: \\
...
\begin{tcolorbox}[solution]
    ...
    The first method is to split the graph into two parts: one part above the $x$-axis and one part below the $x$-axis. We can integrate the part above the $x$-axis as usual, and for the part below the $x$-axis, we simply negate the result of the definite integral. The sum of these two parts should equal the area under the curve. We can begin by finding the point where the graph passes the $x$-axis. \begin{align*}
        ...
        x &= 1.253
    \end{align*}
    Now, by the rule $\int_a^b f(x) \,\mathrm{d}x = \int_a^c f(x) \,\mathrm{d}x + \int_c^b f(x) \,\mathrm{d}x \forcespace$, where $a < c < b$, we can split our integral into two parts. Remember that we are negating the second term to compensate for the area being below the $x$-axis. \begin{align*}
        \int_0^{\sqrt{\pi}} x\cos \left(x^2\right) \,\mathrm{d}x \rightarrow \int_0^{1.253} x\cos \left(x^2\right) \,\mathrm{d}x + \left(-\int_{1.253}^{\sqrt{\pi}} x\cos \left(x^2\right) \,\mathrm{d}x\right)
    \end{align*}
    ...
    However, there is a much cleaner way to do this problem. As it turns out, if we simply take the integral of the absolute value of the integrand (the function that is being integrated), we arrive at the same answer. This is because the absolute value automatically accounts for both the parts of the graph that are above and below the $x$-axis, turning any negative “signed area” into positive area. Therefore, we can write \begin{align*}
        \int_0^{\sqrt{\pi}} \left|x\cos \left(x^2\right)\right| \,\mathrm{d}x, 
    \end{align*}
...
\begin{tcolorbox}[solution]
    ...
    It's clear that the graph crosses the $x$-axis at $x = 1$. Therefore, to get the total area, let's set up our two-term integration and solve. \begin{align*}
        & \int_{-1}^1 \left(x^3 - 2x^2 + 5x + 6\right) \,\mathrm{d}x + \left(-\int_1^2 \left(x^3 - 2x^2 + 5x + 6\right) \,\mathrm{d}x \right) \\[11pt]
...
\begin{tcolorbox}[solution]
    ...
    Note that there are three regions here, so we will need to separate our integral into three terms. Our zeroes between $\left[-1, \, 2\right]$ are $0$ and $\sqrt{2}$. \begin{align*}
        & \int_{-1}^0 \left(x^3 - 2x\right) \,\mathrm{d}x + \left(-\int_0^{\sqrt{2}} \left(x^3 - 2x\right) \,\mathrm{d}x\right) + \int_{\sqrt{2}}^2 \left(x^3 - 2x\right) \,\mathrm{d}x \\[11pt]
...
Therefore, the total mileage that you've covered is $3 + 3 + 6 = 12$ miles. These two questions are the root behind the concepts of $\textbf{displacement}$ and $\textbf{distance}$. \par
...
\begin{tcolorbox}[definition]
    ...
    \begin{tabbing}
        \textit{Distance} $\rightarrow$ \= Definition: How far you travel in total. This value can only be \\ 
        \> positive.
    \end{tabbing} 
\end{tcolorbox}
...
\begin{center}
    \fbox{\fbox{\begin{minipage}{0.96\textwidth}
        ...
        \vspace{11pt}
        $\hfill \text{Displacement} \rightarrow \int_a^b v \,\mathrm{d}t \hfill \text{Total Distance} \rightarrow \int_a^b |v| \,\mathrm{d}t \hfill$ \par
        \vspace{11pt}
        $\hfill \text{Position at } t = a \rightarrow x(a) + \int_a^b v \,\mathrm{d}t \hfill$
        \vspace{11pt}
    \end{minipage}}}
\end{center}
...
\begin{tcolorbox}[solution]
    \textbf{Sol 3.3.4: } \par
    a) \begin{align*}
        \int_a^b v \,\mathrm{d}t & = \int_1^4 \left(t^2 + t - 6\right) \,\mathrm{d}t \\[11pt]
        & = \left[\dfrac{t^3}{3} + \dfrac{t^2}{2} - 6t\right]_1^4 \\[11pt]
        & = \boxed{-\dfrac{21}{2} \si{meters}}
    \end{align*}
    b) $v(t) = 0$ when $t = 2$. \begin{align*}
        \int_a^b |v| \,\mathrm{d}t & = -\int_1^2 \left(t^2 + t - 6\right) \,\mathrm{d}t + \int_2^4 \left(t^2 + t - 6\right) \,\mathrm{d}t \\[11pt]
...
\begin{questions}
    ...
    \end{center}
    In terms of $A$ and $B$, $\int_{-5}^3 f(x) \,\mathrm{d}x + \int_{-2}^3 f(x) \,\mathrm{d}x = $ \\
    ...
    \begin{center}
        \includegraphics[width = 0.6\textwidth]{\graphicsdir Chapter 3 Graphics/3.3-Graphic5.png}
    \end{center}
    In terms of $A$ and $B$, $2\int_{-5}^3 f(x) \,\mathrm{d}x + 3\int_{-2}^3 f(x) \,\mathrm{d}x = $ \\
